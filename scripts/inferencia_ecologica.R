### Inferencia Ecologica para las elecciones a Presidente 2021 Perú
...
data1<-read.csv("data/base_inferencia_ecologica_Peru2021.csv")


dsPpre <- data1[,3:12] #De que col a que col van los rtados de las elecciones prim vuelta
dsPpre <- dsPpre/data1[,2] #n de col de electores
dsPpre <- cbind(dsPpre,1-rowSums(dsPpre))
dsPpre <- as.matrix(dsPpre)

dsGpre <- data1[,13:14] #de que col a que col van los rtados de las elecciones 2da vuelta
dsGpre <- dsGpre/data1[,2] #n de col de electores
dsGpre <- cbind(dsGpre,1-rowSums(dsGpre))
dsGpre<- as.matrix(dsGpre)

#Paso a General
newdata<-as.matrix(cbind(dsPpre,dsGpre))
estsPG<-params.estim(newdata,nR=11,nC=3) #10 filas (1 para blancos/ausentes), 3 cols (1 para blancos/ausentes)
fracsPG<-calc.fractions(estsPG,nR=11,nC=3)
colnames(fracsPG)<-colnames(dsGpre)
rownames(fracsPG)<-colnames(dsPpre)

##Datos Summary
VotosPaso <- data1[,3:12]
VotosPaso <- cbind(VotosPaso,data1[,2]-rowSums(VotosPaso))
VotosGeneral <- data1[,13:14]
VotosGeneral <- cbind(VotosGeneral ,data1[,2]-rowSums(VotosGeneral))
...
indice <- tibble(indice = c("Castillo (PL)", "Fujimori (FP)",
...
tabla %>%
  ...
  rename(NoVoto_Blanco = `1 - rowSums(dsGpre)`) %>%
  write_csv("data/inferenciaEcoResultados.csv")
